# Running unsigned caspar-AV builds
...
caspar-AV listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of caspar-AV**
...
domain. Caspar-AV needs this to serve the web console and reach your CasparCG server over AMCP and OSC.

If you deny it (or the prompt appears behind another window and times out), the console won't load and cues will never reach CasparCG
...
**Private**. If caspar-AV isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "caspar-AV" -Direction Inbound -Program "C:\Path\To\caspar-AV.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*caspar-AV*" | Remove-NetFirewallRule
```
...
```sh
codesign --force --deep --sign - "/Applications/caspar-AV.app"
```

Verify it took:

```sh
codesign -dv --verbose=4 "/Applications/caspar-AV.app"
spctl -a -vv "/Applications/caspar-AV.app"   # still reports "rejected" — ad-hoc is not notarization
```
